...
public class Backup extends Utils
	...
	public Backup()
		...
		try {
			...
			for(File f : directory_root.listFiles()){
				Long date_from = new SimpleDateFormat("yyyy-MMM-dd--HH-mm-ss").parse(f.getName().replace(".sql","")).getTime();
				// check if backup is more than 5 days old, if so delete
				if(date_from < now - (86400000 * 5)){
					f.delete();
				}
        	}
            ...
            if (exit_val == 0){
            	log("Backup success: " + output_file);
            }
